...
- Sprint 7–10: **scope oficial `@design-systems-orion`** (org `portais-orion`, npm público — sem token pra instalar). `@grupo`, `@mateusarcestr`, `@supertrans-transportes` = **legado** (não usar em novos projetos; não apagar sem autorização). **Supertrans é o 1º consumidor oficial** (`/configurador/permissions`, `/configurador/app-shell-canary`). Packages 0.1.0 source-based; **dist/tsup + Changesets** preparado p/ 0.1.1 (dev usa `exports`→src; publish usa `dist` via `publishConfig`). Novos consumidores: `docs/adoption/consumer-setup.md`. Migração de telas: skill `ai/skills/portais-orion-adoption`. **Aurora não é alvo imediato** (depende de preparo próprio + Turborepo, outro dev).
