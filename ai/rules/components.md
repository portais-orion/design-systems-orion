...
�S& Correto (padrão do repo � ver button.tsx):
```tsx
...
- Variants com semântica de domínio (ex.: `origin-profile`, `origin-group` no badge do Supertrans) NÒO sobem para o núcleo � manter apenas variants semânticas (`tinted`, `success`, `warning`).
