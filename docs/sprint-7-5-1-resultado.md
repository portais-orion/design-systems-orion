...
**`@supertrans-transportes`** (confirmado). Working tree ainda em `@grupo` (placeholder); o
...
## Rename de `@grupo` para `@supertrans-transportes`
...
Ainda em `@grupo` (mudam no Passo 2 do runbook, na máquina do dev):
- `packages/{tokens,ui,blocks,tsconfig,biome-config}/package.json` (names + dep interna)
- `packages/**/src/**` (imports `@grupo/ui`), `apps/storybook/**`, `tsconfig` `extends`
