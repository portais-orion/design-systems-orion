...
> **Corre��o (Sprint 7.5.2):** o scope `@supertrans-transportes` foi abandonado (espec�fico demais).
> Scope final = **`@portais-orion`** (org `portais-orion`). Este documento cont�m texto
> auto-referencial corrompido por um `sed` global em locale n�o-UTF-8; ver
...
dos pacotes NÒO renomeados** (gated + não verificável � runbook Passo 2).
