...
## Decisão
`@base-ui/react` é o único primitivo headless permitido em `@grupo/ui` e `@grupo/blocks`. `@radix-ui/*` é proibido. Componentes que existem só no Aurora (tabs, popover, dropdown-menu, switch...) serão recriados em Base UI, nunca portados do código Radix.
