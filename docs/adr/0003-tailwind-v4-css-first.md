...
## Decisão
Tailwind v4 CSS-first em todos os packages e produtos. Configuração de tema vive em CSS (`@grupo/tokens`), nunca em `tailwind.config.ts`. Consumidores usam `@source` para escanear classes dos packages.
...
## Critérios de aceite
Storybook renderiza componentes com classes v4 resolvendo tokens de `@grupo/tokens`.
