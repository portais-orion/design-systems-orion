...
## Decisão
Componentes usam apenas tokens semânticos (`bg-primary`, `text-muted-foreground`). Marcas são arquivos CSS em `@supertrans-transportes/tokens/themes/*` que redefinem variables sob `:root` (fallback) e `[data-brand="<marca>"]`. Proibido: `brand` como prop de componente, hex em componente, classes de marca (`orange-*`, `primary-600`).
