...
`DataTable` implementado em `@supertrans-transportes/blocks/data-table` como o primeiro componente "melhor dos dois": API `Column<T>`/`keyExtractor` do Aurora, markup/tokens do Núcleo (Table do Supertrans), TanStack Table como motor interno. 12 stories (Storybook: 99), documentação com validação contra 8 referências reais dos dois portais, todas as validações e o check de pureza verdes. Nenhum portal alterado.
