...
Camada de composição de **forms e layouts de página** entregue no `@grupo/blocks`: 12 blocks novos (5 de formulário + 7 de layout), 4 stories integradas de página completa e 2 documentos de arquitetura (`forms.md`, `page-layouts.md`). Storybook saltou de 155 para **215 stories** (+60). Uma tela nova agora se monta 100% com padrões oficiais — o princípio "a biblioteca sabe como montar a tela, não o que ela significa" está aplicado em todos os slots. Nenhum portal alterado; validações e pureza verdes.
...
Agnósticos de form library: **zero import de RHF/Zod** (critério 17 ✓).
...
Conforme as seções 7 e 8 do plano, sem desvios de assinatura. Únicos acréscimos: `id` no FormMessage (para aria-describedby) e `contentClassName` no PageLayout já previsto.
