...
2. **Supertrans consome primeiro**: `components/ui/*` viram re-exports de `@grupo/ui`; `globals.css` importa `@grupo/tokens`. Valida o pipeline no portal mais alinhado (já é TW4 + Base UI). Zero mudança visual esperada.
