# Blocks (@grupo/blocks) — Camada 2

## Diferença entre @grupo/ui e @grupo/blocks

`@grupo/ui` (Camada 1) são primitives: elementos de interface indivisíveis (button, input, dialog), estilizados sobre Base UI/nativos. `@grupo/blocks` (Camada 2) são **composições genéricas** desses primitives que resolvem padrões recorrentes de tela (estado vazio, paginação, cabeçalho de página, confirmação). Blocks usam ui; ui nunca usa blocks.
...
| Pagination | Aurora `ui/Pagination.tsx` | Select do @grupo/ui; Intl.NumberFormat pt-BR |
