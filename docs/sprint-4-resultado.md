...
Cobertura do `@supertrans-transportes/ui` ampliada de 19 para **26 primitives** (+popover, radio-group, scroll-area, sheet, accordion, progress, spinner), `LoadingOverlay` criado no `@supertrans-transportes/blocks`, e spike de inputs avançados concluído com decisão documentada. Storybook: **133 stories** (+34). Todas as validações e o check de pureza verdes. Nenhum portal alterado.
