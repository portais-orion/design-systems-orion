...
`Combobox` e `MultiSelect` implementados em `@grupo/ui` sobre o **Combobox do Base UI** (single e `multiple`) — sem cmdk, sem Radix, zero domínio. O MultiSelect é o componente único que substitui conceitualmente `multi-select` (10 telas) e `GroupedMultiSelect` (9 telas) do Aurora. 22 stories novas (Storybook: **155**), com `fn()` nos callbacks e play functions de teclado/interação. Inventário de primitives do Aurora: **100% coberto** (28 componentes no @grupo/ui). Todas as validações e pureza verdes; nenhum portal alterado.
