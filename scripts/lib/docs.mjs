/*
 * As decisões de documentação que não tocam o disco: extração de stories por
 * AST, montagem do snippet copiável, tabela de props e renderização das
 * páginas MDX, do meta.json e do registry.
 *
 * O gerador (`scripts/generate-docs.mjs`) é o adapter: lê os fontes, chama o
 * react-docgen-typescript e escreve os arquivos. Aqui só entram funções puras
 * — é o mesmo seam que `brand-catalog` e `package-distribution` já usam, e o
 * que permite testar a saída sem tocar em `apps/docs`.
 */
 ...
 */
export function extractStories(storySource) {
 ...
 */
export function storyCodeSnippet(story, compName, storySource, pkg, name, exportNames) {
	const sourceFile = ts.createSourceFile(
		"stories.tsx",
		storySource,
		ts.ScriptTarget.Latest,
 ...
 */
export function extractExportNames(sourceContent, compName) {
...
export function formatImport(names, specifier) {
...
/**
 * Converte a saída bruta do react-docgen-typescript nas linhas da tabela.
 * O gerador é quem chama o docgen (ele lê o disco); o formato da linha é
 * decidido aqui.
 */
export function toPropRows(parsedComponents) {
	const rows = [];
 ...
 */
export function extractComponentDoc(sourceContent, compName) {
...
export function renderPropsTable(props, compName, pkg) {
	if (props.length === 0) {
		return [
			"## API Reference",
			"",
...
/**
 * A página MDX de um componente, a partir do que já foi lido do disco:
 * `sourceContent` (o .tsx), `storySource` (o .stories.tsx) e `props` (linhas
 * de `toPropRows`). `packageLabel` e `isGallery` vêm do catálogo do gerador.
 */
export function renderComponentPage({
	...
	const description = docText
		? firstSentence
		: `Componente de ${category.toLowerCase()} da biblioteca ${packageLabel}.`;
...
export function humanizeStoryName(name) {
...
/** meta.json da sidebar: os nomes agrupados na ordem de `categoryOrder`. */
export function renderMeta(title, names, categories, categoryOrder) {
